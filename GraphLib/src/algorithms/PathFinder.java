...
public class PathFinder {
	...
	public static  <V,W extends Number & Comparable<W>> List<WeightedDirectionalEdge<V,W>> lazyDikstrasDouble(WeightedAdjacencyListDiGraph<V,W> graph, V source, V sink){
		ArrayList<WeightedDirectionalEdge<V,W>> path = new ArrayList<>();  // return path;
	...
	private static class VertexInt<V,T extends Number & Comparable<T> ,W> implements Comparable<VertexInt<V,T,W>>{
		...
		VertexInt(V vertex, T val,WeightedDirectionalEdge<V,W> edge) {
			this.vertex = vertex;
			this.val = val;
			this.edge = edge;
		}
